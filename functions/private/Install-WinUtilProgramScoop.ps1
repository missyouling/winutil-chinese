function Install-WinUtilProgramScoop {
    ...
    $process = Start-Process -FilePath scoop -ArgumentList $arguments -NoNewWindow -PassThru
    $exited = $process.WaitForExit($TimeoutSeconds * 1000)
    if (-not $exited) {
        $process.Kill()
        Write-WinUtilLog -Level "ERROR" -Component "Package" -Message "$Action scoop package(s) TIMEOUT: $($Programs -join ', ') (exceeded ${TimeoutSeconds}s)"
        if ($null -ne $FailedPackages) {
            foreach ($p in $Programs) {
                [void]$FailedPackages.Add("$p (scoop:超时)")
            }
        }
    } else {
